...
function createRef(refVal, id) {
    ...
    removeBtn.setAttribute('class', 'mainbutton')
...
for (let index = 0; index < webList.length; index++) {
    ...
    createRef(element, index)
}

function removeLink(i) {
    var filtered = webList.filter(function (value, index, webList) { return value != webList[i]; });
    console.log(webList[i])
    localStorage.setItem("webList", filtered)
    location.reload();
}
...
{
    webList.push(link);
    localStorage.setItem("webList", webList)
    location.reload();
}
